...
# Filter for relevant Product Manager jobs ONLY
print("\nFiltering for Product Manager roles...")
filtered = []
exclude_keywords = [
    "junior", "intern", "entry level", "associate", "coordinator",
    "marketing product", "sales", "account manager", "customer success",
    "project manager", "program manager", "scrum master", "agile coach",
    "business analyst", "data analyst", "operations", "support"
]

for job in all_jobs:
    title_lower = job['title'].lower()
    
    # Must have "product" in title
    if "product" not in title_lower:
        continue
    
    # Must have manager/owner/lead/director
    if not any(word in title_lower for word in ["manager", "owner", "lead", "director"]):
        continue
...
print(f"After filtering: {len(filtered)} relevant jobs")
